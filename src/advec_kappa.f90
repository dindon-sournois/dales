...
subroutine hadvecc_kappa(a_in,a_out)
  ...
  !$acc parallel loop collapse(3) reduction(min:k_low)
  do k = 1, k1
    do j = 2, j1
      do i = 2, i1
  ...
  k_high = 0
  !$acc parallel loop collapse(3) reduction(max:k_high)
  do k = k_low, k1
    do j = 2, j1
      do i = 2, i1
        if ((a_in(i,j,k).ne.0.)) then
...
subroutine  halflev_kappa(a_in,a_out)
    ...
    !$acc parallel loop collapse(3) private(d1,d2,cf) async(1)
    do k = 3, k1
      do j = 2, j1
        do i = 2, i1
          if (w0(i,j,k)>=0) then
    ...
    !$acc parallel loop collapse(2) private(d1,d2,cf) async(2)
    do j = 2, j1
      do i = 2, i1
        if (w0(i,j,2)>=0) then
          d1 = 0
